...
let empty () : ugt =
  { nodes = Hashtbl.create 16; edges = Hashtbl.create 16 }
...
let rec add_many (ids : int list) (graph : ugt) =
  match ids with
  | id :: more ->
      add id graph;
      add_many more graph
...
let connect id1 id2 weight (graph : ugt) : unit =
  ...
  else if not @@ Hashtbl.mem graph.nodes id2 then
    raise (UnknownNode id2)
...
let neighbors graph id =
  ...
  else Hashtbl.find graph.nodes id
