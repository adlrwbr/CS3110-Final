...
(** [inter_coord_opt r1 r2] is the coordinate pair of the intersection between
    ...
    Note: assumes roads have nonzero length*)
  ...
  | ( ((start1_x, start1_y), (end1_x, end1_y)),
      ...
      match
        if start1_x = end1_x && start2_x <> end2_x then
          ...
          if(start1_y = end2_y) 
            then start1_x, end2_y
          else if(start1_y = start2_y)
            then start1_x, start1_y
          else if(end1_y = end2_y) 
            then start1_x, end2_y
          else if(end1_y = start2_y)
            then start1_x, start1_y
          ...
          if m1 = m2 then 
            ...
            let starts_align, ends_align, start_to_end, end_to_start, either_contained = 
            ...
            || start2_x <= start1_x && end2_x >= end1_x) in 
            (
            if either_contained then failwith "No intersection: subset roads"
            else if starts_align && ends_align || start_to_end && end_to_start 
            ...
            else failwith "No intersection: parallel but not overlapping"
            )
          else
            let x =
