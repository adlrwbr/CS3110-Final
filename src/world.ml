...
let empty name =
  { name; roads = []; intersections = []; locations = [] }
...
let road_of_json j =
  ...
  let parse_list (coords : float list) : float * float =
    if List.length coords <> 2 then
      failwith "Coordinate list must consist of two (x, y) float values"
    else (List.nth coords 0, List.nth coords 1)
  in
  let startPt =
    j |> member "start" |> to_list
    |> List.map (fun m -> to_number m)
    |> parse_list
  in
  let endPt =
    j |> member "end" |> to_list
    |> List.map (fun m -> to_number m)
    |> parse_list
  in
...
let intersect_of_json (roads : Road.t list) j =
  ...
  try
    ...
    | None ->
        failwith (r1_name ^ " and " ^ r2_name ^ " do not intersect")
    | Some inter -> inter
  with Not_found ->
    failwith "Road specified in intersections must also be in roads"
...
let loc_of_json (roads : Road.t list) j =
  ...
  let road =
    try List.find (fun r -> Road.name r = road_name) roads
    with Not_found ->
      failwith "Road specified in location must also be in roads"
  in
  {
    id = next ();
    name;
...
let world_of_json j =
  ...
  let roads = j |> member "roads" |> to_list |> List.map road_of_json in
  {
    ...
    roads;
    intersections =
      j |> member "intersections" |> to_list
      |> List.map (intersect_of_json roads);
    locations =
      j |> member "locations" |> to_list |> List.map (loc_of_json roads);
  }
...
let to_json (world : wt) =
  `Assoc
    [
      ...
      ( "intersections",
        `List (List.map intersect_to_json world.intersections) );
...
let add_loc name category road pos world =
  (* create location *)
  let new_loc =
    { id = next (); name; category; road; pos_on_road = pos }
  in
  (* add location to world *)
  let new_world =
    { world with locations = new_loc :: world.locations }
  in
  (new_loc, new_world)

let delete_loc world loc =
  {
    world with
    locations = List.filter (fun l -> l != loc) world.locations;
  }

let add_road (road : Road.t) world =
  let road_name = Road.name road in
  if world.roads |> List.exists (fun r -> Road.name r = road_name) then
    raise (RoadNameConflict road_name)
  else
...
let delete_road world road =
  {
    ...
    intersections =
      List.filter
        (fun (i : Road.it) -> i.road1 != road && i.road2 != road)
        world.intersections;
  }
...
(**[nearest_pt_on_line fix ref_line] is the nearest point on [refline]
   to [fix] or none if the perpendicular does not intersect. (fix beyond
   endpoint) *)
let nearest_pt_on_line fix ref_line =
  let (s1, s2), ((p_a1_x, p_a1_y), (p_a2_x, p_a2_y)) =
    (fix, Road.road_coords ref_line)
  in
  let m_a =
    -.(try Algo.slope p_a1_y p_a1_x p_a2_y p_a2_x
       with Algo.UndefinedSlope -> 1000.)
  in
  let source1 = (s1 -. 10000., s2 -. (10000. *. m_a)) in
...
let near_road source world =
  ...
  let ordp =
    match insct with
    | Some (x, y) ->
        if Algo.distance source (x, y) < Algo.distance source c1 then
          if Algo.distance source (x, y) < Algo.distance source c2 then
            (x, y)
          else c2
        else if Algo.distance source c1 < Algo.distance source c2 then
          c1
        else c2
    | None ->
        if Algo.distance source c1 < Algo.distance source c2 then c1
        else c2
  in
  let interp_dist = Algo.distance ordp c1 /. Algo.distance c1 c2 in
  (interp_dist, minrd)
...
(** [closest loc intersections_locs] is the closest
    intersection/location (whichever is closer) in [intersections_locs]
    to [loc] *)
let closest (coord : float * float) (intersections_locs : lit list) :
    lit =
  List.fold_left
    (fun (closest : lit) (inter_loc : lit) ->
      if
        Algo.distance coord (lit_coord inter_loc)
        < Algo.distance coord (lit_coord closest)
      then inter_loc
      else closest)
    (List.hd intersections_locs)
    intersections_locs
...
let nextdoor_neighbors (world : wt) (node : lit) : lit list =
  let helper coord pos_on_road road =
    ...
    let inters_on_road =
      List.filter
        (fun (inter : Road.it) -> r = inter.road1 || r = inter.road2)
        world.intersections
    in
    ...
    let inters_above =
      List.filter
        (fun (inter : Road.it) ->
          let pos =
            if inter.road1 = r then inter.pos_on_road1
            else inter.pos_on_road2
          in
          pos > pos_on_road)
        inters_on_road
    in
    ...
    let locs_on_road =
      List.filter (fun (l : lt) -> r = l.road) world.locations
    in
    (* locations on the same road as loc, closer to the end *)
    let locs_above =
      List.filter
        (fun (l : lt) -> l.pos_on_road > pos_on_road)
        locs_on_road
    in
    let locs_below =
      List.filter
        ...
        locs_on_road
    in
    (* Convert all intersections and locations into lit type and merge
       them into one list *)
    let above =
      List.map (fun i -> Inter i) inters_above
      @ List.map (fun l -> Loc l) locs_above
    in
    let below =
      List.map (fun i -> Inter i) inters_below
      @ List.map (fun l -> Loc l) locs_below
    in
    (* Check to see if there are nodes above and below. For the cases
       where there are no nodes above/below, do not attempt to find the
       closest location/intersection. *)
    (if List.length above = 0 then [] else [ closest coord above ])
    @ if List.length below = 0 then [] else [ closest coord below ]
  in
  let neighbors =
    match node with
  ...
  in
  neighbors |> List.sort_uniq compare
...
let reduce_aux (world : wt) : Graph.ugt * (int, lit) Hashtbl.t =
  ...
  (* [spread acc queue] is a fully formed graph. Every node in [queue]
     has been "seen" or added to the graph [acc]. *)
  let rec spread (acc : Graph.ugt) (queue : lit list) : Graph.ugt =
    if not (List.for_all (fun n -> Hashtbl.mem seen n) queue) then
      failwith
        "Loop invariant: all nodes in queue must have been added to \
         the graph"
    else
      match queue with
      | [] -> acc
      | cur :: t ->
          ...
          let _ =
            List.map
              (fun n ->
                (* if neighbor hasn't been seen, add to seen, graph, and
                   append to queue *)
                ...
                let distance =
                  Algo.distance (lit_coord cur) (lit_coord n)
                in
  ...
  (* "Total nodes seen: " ^ (Hashtbl.length seen |> Int.to_string) |>
     print_endline; *)
  (result, id_to_lit)

(** [reduce world] is a tuple containing a graph representing the
    simplified [world] where intersections and locations are nodes
    connected by edges (road segments) and a map from that graph's ids
    to the [world]'s [lit]s *)
let reduce (world : wt) : Graph.vgt * (int, lit) Hashtbl.t =
  ...
  let has_inter road =
    List.exists
      (fun (inter : Road.it) ->
        inter.road1 = road || inter.road2 = road)
      world.intersections
  in
  ...
  if List.length world.roads > 1 && isolated then
    ...
    if
      Graph.size unverified
      != List.length world.intersections + List.length world.locations
    then
      failwith
        ("Invalid world! There are "
        ^ (List.length world.intersections |> Int.to_string)
        ^ " intersections and "
        ^ (List.length world.locations |> Int.to_string)
        ^ " locations, but the reduced graph has "
        ^ (Graph.size unverified |> Int.to_string)
        ^ " nodes.")
      ...
      failwith
        ...
        ^ " intersections and "
        ^ (List.length world.locations |> Int.to_string)
        ^ " locations, but after reducing the map id_to_lit has "
        ^ (Hashtbl.length id_to_lit |> Int.to_string)
        ^ " bindings.") (* verify the graph *)
...
let directions world start finish =
  ...
  match reduce world with
  | exception _ ->
      failwith "Cannot calculate directions for invalid world!"
  | graph, id_to_lit ->
      let key_val_pairs = id_to_lit |> Hashtbl.to_seq |> List.of_seq in
      let start_id =
        List.find (fun (_, node) -> node = Loc start) key_val_pairs
        |> fst
      in
      let finish_id =
        List.find (fun (_, node) -> node = Loc finish) key_val_pairs
        |> fst
      in
